...
/**
 *
 * @author 48793 this class represents a set of blocks under one shader. 1 Layer
 * = 1 draw call for this chunk example: layer 1 = all static blocks layer 2 =
 * water blocks layer 3 = lava blocks
 */
public class ChunkLayer {
    ...
    public ChunkLayer(BlockWorld bw, int x, int z, boolean animated) {
        this.bw = bw;
        chunkPos = new Vector2f(x, z);
        this.isAnimated = animated;
    }

    public final Geometry generateGeometry() {
        if (vertexCount == 0) {
            return null;
        }
        Mesh m = generateMesh();
